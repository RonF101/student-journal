...
Route::get('/', function () {
    $hasWriterRole = Schema::hasTable('roles')
        && Role::query()
            ->where('name', 'writer')
    ...
    $landingStats = [
        'studentWriters' => $hasWriterRole ? User::role('writer')->count() : 0,
        'publishedArticles' => Schema::hasTable('articles')
            ? Article::query()
    ...
    $recentArticles = Schema::hasTable('articles')
        ? Article::query()
            ->with([
                'author:id,name',
                'category:id,name',
